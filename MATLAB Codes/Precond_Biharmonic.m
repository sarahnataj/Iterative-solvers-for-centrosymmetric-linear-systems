...
%
% Here A is 2D fourth order spectral differentiation (pseudospectral) matrix
% Preconditioners are:
...
%       precase=3; M=dense spart of A approximated by finite difference
...
[A,C,M]=PseudoSpectral_Biharmonic_varcoef(n,k);AA=A;
G=PseudoSpectral_Poisson_varcoef(n,2,k);
[R,S]=scaling(A);
b=randn(size(A,1),1);
...
condition=condest(AA)
disp('Condition number of the matrix after equlibration++++++++++');
condest(A)
...
if precase==1
    ...
    disp('GMRES with ilu factors of G as preconditioner+++++++++++++');
    [L,U]=ilu(G);
    density_ilu=(nnz(L)+nnz(U)-size(G,1))/nnz(G)
    [x3,fl3,rr3,it3,rv3] = gmres(A,b,[],tol,maxit,@(x)mfun(x,L,U),[],x0);
    it3(2),
    %
    disp('GMRES with ixy factors of G as preconditioner++++++++++++++');
    [X,Y]=ixy(G);
    ...
    semilogy(0:length(rv3)-1,rv3/norm(U\(L\(U\(L\b)))),'-s',...
        ...
        'LineWidth',1.15);
    yline(tol,'r--');
    legend('ilu(G)','ixy(G)','ilutp(G)','ixytp(G)',...
...
elseif precase==2
    ...
    semilogy(0:length(rv5)-1,rv5/norm(UC\(LC\b)),'-s',...
        ...
        'LineWidth',1.15);
    yline(tol,'r--');
    legend('ilu(C)','ixy(C)', 'ilutp(C)', 'ixytp(C)', ...
        'Tolerance','Location','NorthEast')
    xlabel('Iteration number')
    ylabel('Relative residual')
    title('2D biharmonic equation')
...
elseif precase==3
    ...
    semilogy(0:length(rv7)-1,rv7/norm(UM\(LM\b)),'-s',...
        ...
        'LineWidth',1.15);
    yline(tol,'r--');
    legend('ilu(M)','ixy(M)', 'ilutp(M)','ixytp(M)',...
        'Tolerance','Location','NorthEast')
    xlabel('Iteration number')
    ylabel('Relative residual')
    title('2D biharmonic equation')
