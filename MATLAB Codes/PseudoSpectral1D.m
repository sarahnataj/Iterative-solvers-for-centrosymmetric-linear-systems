...
% Output: A is the 1D second/fourth spectral differentiation matrices, and
...
if  order==2
    ...
    A=D*D;
    A=A(2:n,2:n);
    uex=sin(pi*node(2:n)); %f=-pi^2*sin(pi*node(2:n)); %norm(A*uex-f)
elseif order==4
    %FOURTH ORDER
    B2=(diag(1-node.^2)*D^2-8*diag(node)*D-12*eye(n+1))*D^2*diag([0; 1./(1-node(2:n).^2); 0]);
    A=B2(2:n,2:n);
    uex=1+cos(pi*node(2:n)); %f=pi^4*cos(pi*node(2:n)); %norm(A*uex-f);
end
