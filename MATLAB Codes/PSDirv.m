...
% Calclate 1D first order spectral differentiation (pseudospectral) matrix.
% Input: n is the number of interior collocation nodes.  
%        method 1=Legendre,  2=Cheb
% Output: A = 1D first order spectral differentiation matrix
%        node = collocation nodes.
...
if method==1 % Legendre
    ...
    for i=1:n1 % calculate deriv matrix
        ...
        if i>1 && i<n1 D(i,i)=-nodei/2/(1-nodei*nodei); end
        for j=1:n1 if i ~= j D(i,j)=(-1)^(i+j)*di/d(j)/(nodei-node(j)); end; end;
    end
end
end